        ...
        async function loadSchedule() {
            try {
                ...
                if (result.success && result.data.length > 0) {
                    ...
                    if (quotaBadge) quotaBadge.innerHTML = `<i class="fas fa-fire"></i> Sisa Kuota: ${remainingQuota} Peserta`;

                    const scheduleBadge = document.querySelector('.badge:not(.highlight)');
                    if (scheduleBadge) scheduleBadge.innerHTML = `<i class="fas fa-calendar-alt"></i> Jadwal: ${formattedDate}`;

                    const modalQuota = document.getElementById('modalQuota');
                    if (modalQuota) modalQuota.textContent = `${remainingQuota} Peserta`;
                }
            } catch (error) {
